...
pub(crate) struct PaneMonitor {
    pub(crate) bound_transcript_path: Option<PathBuf>,
    ...
    pub(crate) active_task_ids: std::collections::HashMap<String, String>,
    /// Request IDs already counted for token usage (avoid double-counting).
    pub(crate) counted_request_ids: std::collections::HashSet<String>,
}
...
pub struct ClaudeMonitor {
    pub(crate) inner: Arc<Mutex<HashMap<usize, PaneMonitor>>>,
}
...
pub(crate) fn default_transcript_root() -> Option<PathBuf> {
    dirs::home_dir().map(|home| home.join(".claude").join("projects"))
}

pub(crate) fn normalize_transcript_path_candidate(path: &Path) -> Option<PathBuf> {
    normalize_transcript_path_candidate_with_root(path, &default_transcript_root()?)
}

pub(crate) fn normalize_transcript_path_candidate_with_root(
    path: &Path,
...
pub(crate) fn reset_monitor_session(monitor: &mut PaneMonitor, jsonl_path: Option<PathBuf>) {
...
pub(crate) fn process_event(monitor: &mut PaneMonitor, line: &str) {
...
pub(crate) fn encode_cwd_to_project_name(cwd: &Path) -> String {
    cwd.to_string_lossy()
        .chars()
